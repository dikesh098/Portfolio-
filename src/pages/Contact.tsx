...
export default function Contact() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!supabase) {
        // Supabase not configured — fall back to mailto
        window.location.href = `mailto:gautamdikesh098@gmail.com?subject=${encodeURIComponent(form.subject || 'Portfolio Inquiry')}&body=${encodeURIComponent(`Name: ${form.name}\nEmail: ${form.email}\n\n${form.message}`)}`;
        setStatus('success');
        setForm(EMPTY);
        return;
      }
